...
class IMP:
    ...
    def Nyquist(self,ax,cut=False,name=""):
        ...
        if cut:
            imax=self.icut
        ax.plot(self.R[0:imax],-self.X[0:imax],"o",linewidth=1,label=name)
    ...
    def Bode(self,ax1,ax2,name="",cut=False):
        ...
        if cut==True:
            imax=self.icut
        ax1.loglog(self.f[0:imax],np.abs(Z[0:imax]),"o",label=name)
        ax2.semilogx(self.f[0:imax],phi[0:imax],"o",label=name)
    def ftrim(self):
        self.diff()
...
if __name__=="__main__":
    ...
    for k in nums:
        ...
        imax=Z.icut
        ax2.semilogx(Z.f[1:imax],Z.dY[0:imax-1]/Z.dX[0:imax-1],linewidth=3)


    omg=Z.f[0:Z.icut]*2.*np.pi
    ax.set_title(dir)
    ax.legend()
    bx1.legend()
    bx2.legend()
    ...
    n_try=5
    R_try=np.linspace(600,800,n_try)
    C_try=np.linspace(5,15,n_try)
    p_try=np.linspace(0.67,0.70,n_try)
    #for R in R_try:
    #for p in p_try:
    for C in C_try:
        Z5.setup(Rsol,R,C,p)
