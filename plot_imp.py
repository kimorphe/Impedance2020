...
class IMP:
    ...
    def Nyquist(self,ax,cut=False,name=""):
        ...
        if cut:
            imax=self.icut
        ax.plot(self.R[0:imax],-self.X[0:imax],"-k",linewidth=2,label=name)
...
if __name__=="__main__":
    ...
    ax.grid(True)
    ax.set_xlabel(r"Re{Z} [$\Omega$]",fontsize=14)
    ax.set_ylabel(r"Im{Z} [$\Omega$]",fontsize=14)
    ...
    for k in nums:
        ...
        Z.Nyquist(ax,cut=True,name=fname)
        #imax=Z.icut
    ...
    ax.set_aspect(1.0)
    fig.savefig("Nyq_typical.png",bbox_inches="tight")
    fig2.savefig("Bode_typical.png",bbox_inches="tight")
    plt.show()
